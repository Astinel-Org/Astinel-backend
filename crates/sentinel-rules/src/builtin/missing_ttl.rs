...
use sentinel_parser::{ParsedProject, StorageOpKind};
        ...
        p.all_functions()
            ...
            .map(|(f, _)| {
                Finding::new(
                    ...
                    self.category(),
                    f.span.clone(),
                    format!("Function '{}' writes to storage but does not set TTL", f.name),
...
mod tests {
    use super::*;
    use sentinel_parser::{test_helpers::*, FunctionBody, FunctionDef, StorageOpKind, StorageTier, TtlKind};

    #[test]
    fn flags_write_without_ttl() {
        let body = FunctionBody {
            storage_ops: vec![storage_op(StorageOpKind::Set, StorageTier::Persistent, "data", 10)],
            ..Default::default()
        };
    ...
    fn skips_when_ttl_present() {
        let body = FunctionBody {
            storage_ops: vec![storage_op(StorageOpKind::Set, StorageTier::Persistent, "data", 10)],
            ttl_ops: vec![ttl_op(TtlKind::ExtendPersistent, StorageTier::Persistent, true)],
            ..Default::default()
        };
        let func = build_public_fn("store", body);
        let file = build_simple_contract("Test", vec![func], true);
