    ...
    pub fn detect(color_pref: ColorPreference) -> Self {
        let color = match color_pref {
            ...
            ColorPreference::Auto => {
                if std::env::var("NO_COLOR").is_ok() || std::env::var("TERM").is_ok_and(|v| v == "dumb") {
                    false
                } else {
        ...
        let unicode = std::io::stdout().is_terminal() && std::env::var("TERM").map_or(true, |v| v != "dumb");
...
mod tests {
    ...
    fn color_preference_parsing() {
        ...
        assert_eq!(ColorPreference::parse("always"), Some(ColorPreference::Always));
        assert_eq!(ColorPreference::parse("never"), Some(ColorPreference::Never));
