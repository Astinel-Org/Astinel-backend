...
impl SuppressionEngine {
    ...
    fn parse_suppressions(&mut self, path: &Path, content: &str) {
        ...
        self.line_suppressions
            .insert(path.to_path_buf(), line_supps);
        self.file_suppressions
            .insert(path.to_path_buf(), file_supps);
    }
...
mod tests {
    ...
    fn parse_file_level_comment() {
        ...
        std::fs::write(
            &file,
            "// sentinel-ignore-file[test-rule]\nfn foo() {}\nfn bar() {}\n",
        )
        .unwrap();
