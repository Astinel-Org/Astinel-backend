use crate::core::{Rule, RuleRegistry};
...
mod tests {
    use super::*;
    use crate::core::{RuleRegistry, Severity};
    use crate::scanner::rules::testing::TestRuleBuilder;
    ...
    fn all_rules_enabled_by_default() {
        let reg = registry_with(vec![
            Box::new(
                TestRuleBuilder::new("rule-a")
                    .severity(Severity::High)
                    .build(),
            ),
            Box::new(
                TestRuleBuilder::new("rule-b")
    ...
    fn disabled_rule_excluded() {
        ...
        config
            .disabled
            .push(crate::core::RuleId::new("rule-b").unwrap());
    ...
    fn deterministic_order_by_id() {
        let reg = registry_with(vec![
            Box::new(
                TestRuleBuilder::new("z-rule")
                    .severity(Severity::High)
                    .build(),
            ),
            Box::new(
                TestRuleBuilder::new("a-rule")
                    .severity(Severity::High)
                    .build(),
            ),
            Box::new(
                TestRuleBuilder::new("m-rule")
                    .severity(Severity::High)
                    .build(),
            ),
        ]);
